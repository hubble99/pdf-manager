...
def test_exact_empty_outline_is_required():
    for variant, expected in [('safe',True),('painted-space',False),('lying-space-bounds',False),('conflicting-cmap',False)]:
...
@pytest.mark.parametrize('variant',['safe','painted-space','lying-space-bounds','conflicting-cmap','visible-neighbor','clip-mode','duplicate-slot'])
